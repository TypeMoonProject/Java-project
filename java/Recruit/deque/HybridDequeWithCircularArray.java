...
public class HybridDequeWithCircularArray<E> {
    ...
    public HybridDequeWithCircularArray(int arrayCapacity) {
    ...
    public E peekFirst() {
        ...
        if(frontSize != 0) {
            ...
            re = arrayGet((front + 1) % arr.length);
        }
        return re;
    }
    ...
    private boolean arrayIsFull(){
        return (rear) % arr.length == front;
    }

    private void arrayAddFirst(E x) {
        if (arrayIsFull()) throw new IllegalStateException("array part full");
        arr[front] = x;
    ...
    private E arrayRemoveLast() {
        int point = (rear -1 + arr.length) % arr.length;
        E v = (E) arr[point];
        arr[rear] = null;
        rear = point;
        backSize--;
